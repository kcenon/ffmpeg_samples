 ...
 * Webcam Capture
 *
 * This sample demonstrates how to capture video from a webcam using AVDevice.
 * It supports AVFoundation (macOS), V4L2 (Linux), and dshow (Windows).
 */
...
class WebcamCapture {
public:
  WebcamCapture(std::string_view format_name, std::string_view device_name,
    ...
    // Open device
    // Note: For some devices, we might need to pass options (framerate, size)
    // via dictionary
    ffmpeg::check_error(
  ...
  void capture(int duration_seconds) {
    std::cout << std::format("Capturing from {} to {} for {} seconds...\n",
                             input_format_ctx_->url, output_file_,
                             duration_seconds);
    ...
    while (true) {
      ...
      if (packet_->stream_index == video_stream_index_) {
        ...
        av_packet_rescale_ts(
            packet_.get(),
            input_format_ctx_->streams[video_stream_index_]->time_base,
...
int main(int argc, char *argv[]) {
  if (argc < 4) {
    ...
    std::cerr << "Examples:\n";
    std::cerr << std::format("  macOS:   {} avfoundation \"0\" output.mp4\n",
                             argv[0]);
    std::cerr << std::format("  Linux:   {} v4l2 /dev/video0 output.mp4\n",
                             argv[0]);
    std::cerr << std::format(
        "  Windows: {} dshow video=\"Camera Name\" output.mp4\n", argv[0]);
    return 1;
  }

  try {
    int duration = argc > 4 ? std::atoi(argv[4]) : 10;
    WebcamCapture capture(argv[1], argv[2], argv[3]);
