...
public:
  StreamingServer(std::string_view input_file, std::string_view output_url,
      ...
      : input_file_(input_file), output_url_(output_url), format_(format),
  ...
  void start() {
    ...
    ffmpeg::check_error(avformat_write_header(output_ctx.get(), &opts),
                        "write header");
...
private:
  ...
  void stream_packets(AVFormatContext *output_ctx,
                      const std::vector<int> &stream_mapping) {
    ...
    do {
      ...
      while (av_read_frame(format_ctx_.get(), packet_.get()) >= 0) {
        ...
        if (packet_->pts != AV_NOPTS_VALUE) {
          const auto pts_time =
              av_rescale_q(packet_->pts - first_pts, in_stream->time_base,
...
void print_usage(std::string_view prog_name) {
  ...
  std::cout << std::format(
      "    {} video.mp4 http://localhost:8080/stream.flv --format flv\n",
      prog_name);
  std::cout << "\n  RTMP Streaming (requires RTMP server):\n";
  std::cout << std::format(
      "    {} video.mp4 rtmp://localhost/live/stream --format flv\n",
      prog_name);
  std::cout << "\n  UDP Streaming:\n";
  std::cout << std::format(
      "    {} video.mp4 udp://239.1.1.1:1234 --format mpegts\n", prog_name);
  std::cout << "\n  File Output (HLS):\n";
  std::cout << std::format("    {} video.mp4 stream.m3u8 --format hls\n",
                           prog_name);
  std::cout << "\n  Loop Streaming:\n";
  std::cout << std::format(
      "    {} video.mp4 http://localhost:8080/stream --loop\n", prog_name);
  std::cout << "\nNote: Some formats require a running server (RTMP, HTTP) or "
               "client.\n";
  std::cout
      << "For testing HTTP streaming, use a media player like VLC or ffplay:\n";
  ...
  try {
    ...
    if (format.empty()) {
      ...
      if (url_str.starts_with("rtmp://")) {
        format = "flv";
      } else if (url_str.starts_with("udp://") ||
                 url_str.starts_with("rtp://")) {
        format = "mpegts";
    ...
    std::cerr << "\nTroubleshooting:\n";
    std::cerr
        << "- For RTMP: Ensure RTMP server is running (e.g., nginx-rtmp)\n";
