 ...
 * channel layout, or sample format) using modern C++20 and FFmpeg's libswresample.
 */
...
public:
    ...
    void resample() {
        ...
        write_wav_header(output_stream, target_sample_rate_, target_channels_, total_data_size);
...
private:
    void initialize() {
        ...
        // Setup resampler
        AVChannelLayout out_ch_layout = target_channels_ == 1 ?
        ...
        ffmpeg::check_error(
            av_samples_alloc_array_and_samples(&dst_data_, &dst_linesize_, target_channels_,
    ...
    uint32_t resample_and_write(std::ofstream& output_stream) {
        ...
        const auto dst_bufsize = av_samples_get_buffer_size(&dst_linesize_, target_channels_,
...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        std::cerr << std::format("Usage: {} <input_file> <output_file> [sample_rate] [channels]\n", argv[0]);
    ...
    try {
        ...
        AudioResampler resampler(input_filename, output_filename, target_sample_rate, target_channels);
        resampler.resample();
