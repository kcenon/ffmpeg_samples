...
const ReverbParams PRESET_ROOM = {
    .room_size = 0.3,
    .reverberance = 0.5,
    .hf_damping = 0.5,
    .wet_gain = -5.0,
...
const ReverbParams PRESET_HALL = {
    .room_size = 0.8,
    .reverberance = 0.7,
    .hf_damping = 0.5,
    .wet_gain = -3.0,
...
const ReverbParams PRESET_PLATE = {
    .room_size = 0.5,
    .reverberance = 0.5,
    .hf_damping = 0.7,
    .wet_gain = -4.0,
...
const ReverbParams PRESET_SPRING = {
    .room_size = 0.2,
    .reverberance = 0.6,
    .hf_damping = 0.3,
    .wet_gain = -6.0,
...
const ReverbParams PRESET_CATHEDRAL = {
    .room_size = 1.0,
    .reverberance = 0.84,
    .hf_damping = 0.7,
    .wet_gain = -1.0,
...
const ReverbParams PRESET_CHAMBER = {
    .room_size = 0.4,
    .reverberance = 0.6,
    .hf_damping = 0.4,
    .wet_gain = -4.0,
...
private:
    ...
    void setup_filter_graph() {
        ...
        const auto* reverb = avfilter_get_by_name("afreqshift");

        // Check if freeverb is available (it might not be in all builds)
        const auto* freeverb = avfilter_get_by_name("freeverb");
        if (!freeverb) {
            throw std::runtime_error(
                "freeverb filter not found. Please use FFmpeg built with freeverb support.");
        }
        ...
        const auto reverb_args = std::format(
            "room_size={}:reverberance={}:hf_damping={}:wet_gain={}:dry_gain={}:stereo_depth={}",
        ...
        if (params_.pre_delay > 0.0) {
            ...
            const auto delay_args = std::format("delays={}:all=1",
                                                static_cast<int>(params_.pre_delay));
        ...
        for (int i = 3; i < argc; ++i) {
            ...
            if (arg == "--preset" && i + 1 < argc) {
                ...
                if (params.stereo_depth < 0.0 || params.stereo_depth > 100.0) {
                    throw std::runtime_error("Stereo depth must be between 0.0 and 100.0");
                }
            }
        }
