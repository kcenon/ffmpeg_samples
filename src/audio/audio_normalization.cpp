...
void print_usage(std::string_view prog_name) {
  ...
  std::cout << "Options:\n";
  std::cout
      << "  -m, --mode <mode>         Normalization mode (default: peak)\n";
  std::cout << "                              peak     - Peak normalization\n";
  std::cout
      << "                              loudness - EBU R128 loudness (LUFS)\n";
  std::cout
      << "                              rms      - RMS-based normalization\n";
  std::cout << "                              truepeak - True peak limiting\n";
  std::cout << "  -l, --level <dB/LUFS>     Target level (default: -1.0 dB or "
               "-23 LUFS)\n";
  std::cout
      << "  -t, --truepeak <dBTP>     True peak limit (default: -1.0 dBTP)\n";
  ...
  std::cout << std::format(
      "  {} podcast.wav output.wav -m loudness -l -19 -t -1.5\n", prog_name);
  ...
  std::cout << "Notes:\n";
  std::cout
      << "  - Peak normalization: Simple, but ignores perceived loudness\n";
...
NormalizationMode parse_mode(std::string_view mode_str) {
  ...
  if (mode_str == "loudness")
    return NormalizationMode::LOUDNESS;
  if (mode_str == "rms")
    return NormalizationMode::RMS;
  if (mode_str == "truepeak")
...
private:
  ...
  std::string get_mode_name() const {
    ...
    case NormalizationMode::LOUDNESS:
      return "Loudness Normalization (EBU R128)";
    case NormalizationMode::RMS:
      return "RMS Normalization";
    case NormalizationMode::TRUEPEAK:
      return "True Peak Limiting";
    default:
      return "Unknown";
    }
  }
  ...
  void normalize_two_pass() {
    ...
    std::cout << std::format(
        "Analysis complete. Calculated gain: {:.2f} dB\n\n", gain);
  ...
  double analyze_audio() {
        ...
        if (input_frame_->format == AV_SAMPLE_FMT_FLT ||
          ...
          for (int ch = 0; ch < channels; ++ch) {
            const float *samples = reinterpret_cast<float *>(
                input_frame_
                    ->data[input_frame_->format == AV_SAMPLE_FMT_FLTP ? ch
                                                                      : 0]);

            for (int i = 0; i < num_samples; ++i) {
              const int idx = input_frame_->format == AV_SAMPLE_FMT_FLTP
                                  ? i
                                  : i * channels + ch;
    ...
    case NormalizationMode::LOUDNESS: {
      // For loudness normalization, we'd typically use the loudnorm filter's
      // two-pass mode This is a simplified version
      if (total_samples > 0) {
        const double rms = std::sqrt(sum_squares / total_samples);
        if (rms > 0.0) {
          const double estimated_lufs =
              20.0 * std::log10(rms) - 3.0; // Rough estimate
  ...
  void setup_filter_graph(double gain) {
    ...
    case NormalizationMode::LOUDNESS:
      filter_desc =
          std::format("loudnorm=I={}:TP={}:LRA=11", params_.target_level,
