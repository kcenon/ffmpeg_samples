...
void print_usage(std::string_view prog_name) {
    ...
    std::cout << "Options:\n";
    std::cout << "  -t, --threshold <dB>       Silence threshold in dB (default: -40.0)\n";
    std::cout << "  -s, --silence <seconds>    Minimum silence duration (default: 0.5)\n";
    std::cout << "  -m, --min-length <seconds> Minimum segment duration (default: 1.0)\n";
    std::cout << "  -o, --output <directory>   Output directory (default: segments)\n";
    std::cout << "  -f, --format <format>      Output format: wav, mp3 (default: wav)\n";
    std::cout << "  -p, --prefix <prefix>      Output filename prefix (default: segment)\n\n";
    ...
    std::cout << "    Split and save to 'output' directory with prefix 'part'\n\n";

    std::cout << "Notes:\n";
    std::cout << "  - Lower threshold values (e.g., -50dB) detect quieter silence\n";
    std::cout << "  - Increase min-silence to avoid splitting on short pauses\n";
    std::cout << "  - Segments shorter than min-length are merged with adjacent segments\n";
    std::cout << "  - Output files are named: <prefix>_001.<format>, <prefix>_002.<format>, etc.\n";
}
...
public:
    ...
    void split() {
        ...
        auto split_points = detect_split_points(total_duration);
...
private:
    ...
    std::vector<double> detect_split_points(double total_duration) {
        std::vector<double> split_points;

        // For simplicity, we'll split based on silence detection using a simple approach
        // In a real implementation, you would parse silencedetect filter output
        // Here we'll create segments at regular intervals as a demonstration
        ...
        const int min_silence_samples = static_cast<int>(params_.min_silence * sample_rate);
        const int16_t silence_threshold_value = static_cast<int16_t>(32767.0 * std::pow(10.0, params_.noise_threshold / 20.0));
        ...
        while (av_read_frame(input_format_ctx_.get(), input_packet_.get()) >= 0) {
            ...
            while (avcodec_receive_frame(input_codec_ctx_.get(), input_frame_.get()) >= 0) {
                ...
                // Convert to PCM
                const int max_samples = swr_get_out_samples(swr_ctx_.get(), input_frame_->nb_samples);
                ...
                for (int i = 0; i < converted_samples * channels; ++i) {
                    const bool is_silent = std::abs(audio_buffer[i]) < silence_threshold_value;

                    if (is_silent) {
                        if (!in_silence) {
                            in_silence = true;
                            silence_start = static_cast<double>(input_frame_->pts * stream->time_base.num) / stream->time_base.den;
                        ...
                        if (in_silence && silence_sample_count >= min_silence_samples) {
                            ...
                            const double split_time = silence_start + (params_.min_silence / 2.0);
    ...
    void extract_segment(const SplitSegment& segment) {
        const auto output_filename = std::format("{}/{}_{:03d}.{}",
            params_.output_dir, params_.output_prefix, segment.segment_number, params_.output_format);

        std::cout << std::format("Extracting segment {}: {:.2f}s - {:.2f}s -> {}\n",
            segment.segment_number, segment.start_time, segment.end_time, output_filename);

        // Seek to start position
        const auto* stream = input_format_ctx_->streams[audio_stream_index_];
        const int64_t start_pts = static_cast<int64_t>(segment.start_time * stream->time_base.den / stream->time_base.num);

        av_seek_frame(input_format_ctx_.get(), audio_stream_index_, start_pts, AVSEEK_FLAG_BACKWARD);
        avcodec_flush_buffers(input_codec_ctx_.get());
        ...
        while (av_read_frame(input_format_ctx_.get(), local_packet.get()) >= 0) {
            ...
            while (avcodec_receive_frame(input_codec_ctx_.get(), local_frame.get()) >= 0) {
                ffmpeg::ScopedFrameUnref frame_guard(local_frame.get());

                const double frame_time = static_cast<double>(local_frame->pts * stream->time_base.num) / stream->time_base.den;
                ...
                // Convert to PCM
                const int max_samples = swr_get_out_samples(swr_ctx_.get(), local_frame->nb_samples);
                audio_buffer.resize(max_samples * channels);
                ...
                if (converted_samples > 0) {
                    const auto bytes_to_write = converted_samples * channels * sizeof(int16_t);
                    output_stream.write(reinterpret_cast<char*>(audio_buffer.data()), bytes_to_write);
    ...
    segment_complete:
        ...
        std::cout << std::format("  Created: {} ({:.2f}s, {} bytes)\n",
            output_filename, segment.end_time - segment.start_time, total_bytes_written);
    }
