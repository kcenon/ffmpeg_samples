...
void print_usage(std::string_view prog_name) {
    ...
    std::cout << "Options:\n";
    std::cout << "  -t, --threshold <dB>     Limiting threshold in dB (default: -1.0)\n";
    std::cout << "  -a, --attack <ms>        Attack time in milliseconds (default: 5)\n";
    std::cout << "  -r, --release <ms>       Release time in milliseconds (default: 50)\n";
    std::cout << "  -l, --lookahead <ms>     Lookahead time in milliseconds (default: 5)\n";
    std::cout << "  -c, --ceiling <dB>       Output ceiling in dB (default: -0.1)\n";
    std::cout << "  --no-truepeak            Disable true peak detection\n";
    std::cout << "  -p, --preset <name>      Use preset configuration\n\n";

    std::cout << "Presets:\n";
    std::cout << "  mastering   - Mastering limiter (threshold: -1.0dB, ceiling: -0.1dB)\n";
    std::cout << "  broadcast   - Broadcast standard (threshold: -2.0dB, ceiling: -1.0dB)\n";
    std::cout << "  streaming   - Streaming optimized (threshold: -1.5dB, ceiling: -0.5dB)\n";
    std::cout << "  podcast     - Podcast/voice (threshold: -3.0dB, ceiling: -1.0dB)\n";
    std::cout << "  aggressive  - Aggressive limiting (threshold: -0.5dB, ceiling: -0.05dB)\n";
    std::cout << "  gentle      - Gentle limiting (threshold: -3.0dB, ceiling: -1.5dB)\n\n";
    ...
    std::cout << std::format("  {} music.flac output.flac -p streaming --no-truepeak\n", prog_name);
    std::cout << "    Streaming preset without true peak detection\n\n";

    std::cout << "Notes:\n";
    std::cout << "  - Threshold: Level above which limiting is applied\n";
    std::cout << "  - Attack: How quickly limiter responds to peaks (faster = more transparent)\n";
    std::cout << "  - Release: How quickly limiter releases after peak (affects pumping)\n";
    std::cout << "  - Lookahead: Delay for peak detection (prevents overshoot)\n";
    std::cout << "  - True peak: Detects inter-sample peaks (prevents digital clipping)\n";
    std::cout << "  - Ceiling: Maximum output level (safety margin for codecs)\n\n";

    std::cout << "Use Cases:\n";
    std::cout << "  - Mastering: Maximize loudness while preventing clipping\n";
    std::cout << "  - Broadcast: Meet loudness standards (e.g., EBU R128)\n";
    std::cout << "  - Streaming: Optimize for streaming platforms (Spotify, YouTube)\n";
...
private:
    ...
    void setup_filter_graph() {
        ...
        const double ceiling_linear = std::pow(10.0, params_.ceiling / 20.0);

        std::string filter_desc = std::format(
            "alimiter=limit={}:attack={}:release={}:level=1",
            limit_linear,
            params_.attack,
            params_.release
        );
