...
void print_usage(std::string_view prog_name) {
    ...
    std::cout << "Options:\n";
    std::cout << "  -s, --semitones <value>   Pitch shift in semitones (default: 0)\n";
    std::cout << "                              Positive = higher pitch\n";
    std::cout << "                              Negative = lower pitch\n";
    std::cout << "  -t, --no-tempo            Don't preserve tempo (speed will change)\n";
    ...
    std::cout << "  chipmunk      - Chipmunk effect (+12 semitones, no tempo preserve)\n";
    ...
    std::cout << "    Shift 5 semitones without preserving tempo (speed changes)\n\n";

    std::cout << "Notes:\n";
    std::cout << "  - Semitones: Musical interval (12 semitones = 1 octave)\n";
    std::cout << "  - Tempo preservation: Keeps duration same, only changes pitch\n";
...
public:
    ...
    void process() {
        ...
        std::cout << std::format("Pitch Shift: {:.1f} semitones", params_.semitones);
...
private:
    ...
    void setup_filter_graph() {
        ...
        if (params_.preserve_tempo) {
            ...
            const int new_sample_rate = static_cast<int>(input_codec_ctx_->sample_rate * pitch_ratio);
