...
const TremoloParams PRESET_SLOW = {
    .frequency = 2.0, .depth = 0.5, .waveform = "sine"};
const TremoloParams PRESET_FAST = {
    .frequency = 8.0, .depth = 0.6, .waveform = "sine"};
const TremoloParams PRESET_HELICOPTER = {
    .frequency = 15.0, .depth = 0.8, .waveform = "triangle"};
const TremoloParams PRESET_PULSING = {
    .frequency = 4.0, .depth = 0.7, .waveform = "square"};
...
public:
  TremoloProcessor(const std::string &input_file,
                   const std::string &output_file, const TremoloParams &params)
  ...
  void setup_filter_graph() {
    ...
    if (params_.waveform == "sine") {
      ...
      throw std::runtime_error(
          std::format("Invalid waveform: {}", params_.waveform));
    }
    ...
    check_error(avfilter_link(buffersrc_ctx, 0, tremolo_ctx, 0),
                "Linking buffer to tremolo");
