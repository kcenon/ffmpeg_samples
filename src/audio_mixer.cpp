...
public:
    AudioMixer(std::string_view input1, std::string_view input2,
        ...
        : output_file_(output)
        , volume1_(std::clamp(volume1, 0.0f, 1.0f))
    ...
    void mix() {
        ...
        while (!decoder1_.is_eof() || !decoder2_.is_eof()) {
            const auto samples1 = decoder1_.read_samples(buffer1_.data(), buffer_size_);
            const auto samples2 = decoder2_.read_samples(buffer2_.data(), buffer_size_);
            ...
            for (int i = 0; i < max_samples * target_channels_; ++i) {
                const auto sample1 = (i < samples1 * target_channels_) ? buffer1_[i] : int16_t{0};
                const auto sample2 = (i < samples2 * target_channels_) ? buffer2_[i] : int16_t{0};
            ...
            // Write to file
            const auto bytes_to_write = max_samples * target_channels_ * sizeof(int16_t);
            output_stream.write(reinterpret_cast<char*>(output_buffer_.data()), bytes_to_write);
            total_samples_written += max_samples;

            ++iteration;
            if (iteration % 100 == 0) {
                const auto seconds = total_samples_written / static_cast<double>(target_sample_rate_);
                std::cout << std::format("Mixed {:.2f} seconds\r", seconds) << std::flush;
            }
        }
...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        std::cerr << std::format("Usage: {} <input1> <input2> <output> [volume1] [volume2]\n", argv[0]);
        std::cerr << std::format("Example: {} audio1.mp3 audio2.mp3 mixed.wav 0.5 0.5\n", argv[0]);
    ...
    try {
        ...
        const float volume1 = argc > 4 ? static_cast<float>(std::atof(argv[4])) : 0.5f;
        const float volume2 = argc > 5 ? static_cast<float>(std::atof(argv[5])) : 0.5f;

        std::cout << "Audio Mixer\n";
        std::cout << "===========\n\n";
        std::cout << std::format("Input 1: {} (volume: {:.2f})\n", input1_filename, volume1);
        std::cout << std::format("Input 2: {} (volume: {:.2f})\n", input2_filename, volume2);

        AudioMixer mixer(input1_filename, input2_filename, output_filename, volume1, volume2);
        mixer.mix();
